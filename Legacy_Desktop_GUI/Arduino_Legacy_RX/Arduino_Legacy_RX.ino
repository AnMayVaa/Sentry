...
typedef struct {
    ...
    int8_t rssi;
    uint16_t len;
    int8_t buf[128]; // Correct signed type for CSI
    bool is_sos;
} csi_packet_t;
...
void loop() {
    ...
    // 2. Process CSI Queue and send via USB Serial
    csi_packet_t pkt;
    if (xQueueReceive(csi_queue, &pkt, 0) == pdTRUE) {
        if (pkt.is_sos) {
            Serial.printf("SOS_ALERT\n");
        } else {
            // Use Serial.printf() to perfectly emulate the old ESP-IDF C code
            // This prevents Arduino's Serial.print(int8_t) from printing garbage ASCII characters
